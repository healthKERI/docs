...
<Steps>
    ...
    <Step title="Access RACK Gateways">
        Launching the proxies will open two browser windows, one for each RACK instance. The browsers will display the <Tooltip tip="Administration User Interface">AUI</Tooltip> for each gateway.
...
<Steps>
    ...
    <Step title="Create Outbound Routes">
        ...
        **Local Identifier to Secure the Connection**. Leave the **Protocol** set to "HTTP" and set the **Local Port** to 3333.
...
<Steps>
    ...
    <Step title="Edit Outbound Route">
        ...
        Navigate to the **Outbound Routes** section of **Rack1**. Select **Edit** from the **Actions** menu of the
...
**Mirth1** has a channel that is looking for any JSON files to be copied into its local `/opt/connect/data` directory (mapped to the `<rack-docker dir>/examples/data/volumes/mirth1` directory by Docker compose).
